...
function CompanyRegistrationPage() {
  ...
  const handleSocialLinkChange = (platform, value) => {
    ...
    const isValidUrl = platformConfig.regex.test(value.trim());
        ...
        <div className="mt-8 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              ref={fileInputRef}
              accept="image/*"
              className={`mt-1 block w-full rounded-md shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm p-3 border ${errors.comp_profile_img ? "border-red-500" : "border-gray-300"}`}
            />
            {errors.comp_profile_img && (
              <p className="mt-1 text-sm text-red-600">{errors.comp_profile_img}</p>
            )}
            {formData.comp_profile_img && (
              <p className="mt-1 text-sm text-gray-600">Selected: {formData.comp_profile_img.name}</p>
            )}
          </div>
