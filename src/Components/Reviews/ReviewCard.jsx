...
const ReviewCard = ({
  ...
  const handleDeleteReview = async (reviewId, reviewRating) => {
    ...
    // Optimistic update
    removeOptimisticReview(reviewId, reviewRating);
    console.log('🚀 Optimistic review deletion:', reviewId);
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('🚀 Triggering refreshData after deletion...');
        await refreshData();
        console.log('✅ refreshData completed after deletion');
      } else {
        const errorData = await response.json().catch(() => ({}));
      ...
      // Note: Re-adding the review would require storing it temporarily
    } finally {
      setDeletingReviews(prev => {
        const newSet = new Set(prev);
        newSet.delete(reviewId);
  ...
  const isCommentsHidden = hiddenComments[review.id] !== false;
              ...
              <div className="flex flex-wrap gap-2 items-center">
                {(review.text || review.comment) &&
                  (review.text || review.comment).length > 150 && (
                    <button
                      onClick={() => onToggleExpansion(review.id)}
        ...
        <div className="fixed inset-0 backdrop-blur bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl border border-gray-200 max-w-md w-full p-6">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                onClick={() => handleDeleteReview(deleteConfirmation.id, deleteConfirmation.rating)}
                disabled={deletingReviews.has(deleteConfirmation.id)}
