    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 w-full max-w-full">
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start space-y-4 sm:space-y-0 sm:space-x-6 mb-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 mb-2">
              <h3 className="text-lg sm:text-xl font-bold text-gray-900 break-words">{companyData.name}</h3>
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-3 break-words">{companyData.description}</p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 text-xs sm:text-sm text-gray-500">
              ...
              {companyData.gstinNum && (
                <div className="flex items-center justify-center sm:justify-start space-x-1">
                  ...
                  <span className="break-all text-xs">GSTIN: {companyData.gstinNum}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <div className="flex items-start space-x-3">
                ...
                <div className="text-xs sm:text-sm text-gray-600 break-words">
                  ...
                  {(companyData.address.city || companyData.address.state || companyData.address.zipCode) && (
                    <p>{companyData.address.city}{companyData.address.state && `, ${companyData.address.state}`} {companyData.address.zipCode}</p>
                  )}
                  {companyData.address.country && <p>{companyData.address.country}</p>}
                </div>
              </div>
          ...
          {/* Social Media */}
          <div>
            <h4 className="font-medium text-gray-900 mb-3 text-sm sm:text-base">Social Media</h4>
            <div className="space-y-3">
              ...
              {(!companyData.socials?.linkedin && !companyData.socials?.twitter && !companyData.socials?.facebook) && (
                <p className="text-xs sm:text-sm text-gray-500">No social media links added</p>
              )}
            </div>
          </div>
        </div>
