...
export default function Sidebar({ active, setActive }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`sidebar fixed md:relative z-50 flex ${open ? "w-64" : "w-16"} bg-white text-gray-800 min-h-screen transition-all duration-300 shadow-lg border-r border-gray-200 ${
          isMobile && !open ? "translate-x-[-100%]" : "translate-x-0"
        } md:translate-x-0`}
      >
          ...
          <nav className="flex flex-col mt-6 space-y-1 px-2">
            {menuItems.map((item) => (
              <button
                ...
                onClick={() => {
                  setActive(item.id);
                  if (isMobile) setOpen(false); // Close sidebar after selection on mobile
                }}
      ...
      {isMobile && !open && (
        <button
          ...
          className="fixed bottom-4 left-4 z-40 p-3 bg-amber-500 text-white rounded-full shadow-lg md:hidden"
        >
          <Menu size={24} />
        </button>
      )}
