...
export default function CompanyProfile({ companyData = {}, ratingBreakdown = [] }) {
    ...
    useEffect(() => {
        const fetchCompanyData = async () => {
            ...
            try {
                ...
                // Fetch rating stats
                const statsResponse = await fetch(`http://localhost:5000/api/reviews/company/${id}/stats`);
                if (!statsResponse.ok) {
                ...
                const ratingsResponse = await fetch(`http://localhost:5000/api/reviews/company/${id}/ratings`);
                if (!ratingsResponse.ok) {
                    throw new Error('Failed to fetch rating distribution');
                }
                const ratingsData = await ratingsResponse.json();

                // Combine company data with rating stats
                setCompany({
                    ...companyData,
                ...
                // Transform ratings data to match expected format
    ...
    const data = company || companyData;
    const ratings = ratingDistribution.length ? ratingDistribution : ratingBreakdown;
                    ...
                    <div className="bg-gray-50 p-6 rounded-lg shadow-sm border border-gray-200 sticky top-6">
                        ...
                        <div className="mt-8 pt-6 border-t border-gray-300">
                            <div className="text-center bg-white rounded-lg p-4 shadow-xs border border-gray-200">
                                ...
                                <div className="mt-2 flex justify-center">
                                    {renderStars(Math.floor(data.averageRating || 0), "w-4 h-4")}
                                </div>
                            </div>
                        </div>
                    </div>
